...
/// 全局映像请求网络线程，由NSURLConnection委托使用。
...
#pragma mark - Runs in operation thread  (在操作线程中运行)
...
#pragma mark - NSURLConnectionDelegate runs in operation thread  (NSURLConnectionDelegate在操作线程中运行)
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        if (progressiveBlur) { // 仅支持渐进式JPEG和隔行PNG
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                /*
                 如果图像有动画，请将原始图像数据保存到磁盘缓存。
                 如果图像不是png或jpeg，请将图像重新编码为png或jpeg以获得更好的解码性能。
                 */
                ...
                switch (imageType) {
                    ...
                    case YYImageTypeGIF:
                    case YYImageTypePNG:
                    case YYImageTypeWebP: { // 保存到磁盘缓存
                        if (!hasAnimation) {
                            if (imageType == YYImageTypeGIF ||
                                imageType == YYImageTypeWebP) {
                                self.data = nil; // 清除数据，重新编码磁盘缓存
                            }
                        }
                    } break;
                    default: {
...
#pragma mark - Override NSOperation  (重写NSOperation)
